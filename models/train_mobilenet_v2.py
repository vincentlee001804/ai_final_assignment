"""
Training script for Mobilenet V2
...
for epoch in range(num_epochs):
    ...
    os.makedirs(model_dir, exist_ok=True)
    torch.save(model.state_dict(), os.path.join(model_dir, "mobilenet_v2_last.pt"))

    if avg_val_loss < best_val_loss: # Model is getting better/ improving.
        best_val_loss = avg_val_loss
        counter = 0
        torch.save(model.state_dict(), os.path.join(model_dir, "mobilenet_v2_best.pt"))
        print("mobilenet_v2_best.pt has been saved!")
    else:
        counter = counter + 1
