...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), os.path.join(model_dir, "alexnet_last.pt"))

    if avg_val_loss < best_val_loss: # Model is getting better/ improving.
        best_val_loss = avg_val_loss
        counter = 0
        torch.save(model.state_dict(), os.path.join(model_dir, "alexnet_best.pt"))
        print("alexnet_best.pt has been saved!")
    else:
        counter = counter + 1
